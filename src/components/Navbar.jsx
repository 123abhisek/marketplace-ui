...
const ROLE_BADGE = {
  guest:   null,
  free:    { label: 'Free',           bg: 'rgba(109,91,255,0.09)', color: '#5b4cf0', border: 'rgba(109,91,255,0.18)', icon: <WorkspacePremiumRoundedIcon sx={{ fontSize: 13 }} /> },
  premium: { label: 'Premium Active', bg: 'rgba(16,185,129,0.09)', color: '#047857', border: 'rgba(16,185,129,0.20)', icon: <WorkspacePremiumRoundedIcon sx={{ fontSize: 13 }} /> },
  admin:   { label: 'Admin',          bg: 'rgba(239,68,68,0.09)',  color: '#b91c1c', border: 'rgba(239,68,68,0.20)',  icon: <VerifiedUserRoundedIcon    sx={{ fontSize: 13 }} /> },
}
  ...
  return (
    ...
    >
      ...
      <Box>
        ...
        {!compact && (
          <Typography sx={{ fontSize: '0.68rem', fontWeight: 600, color: '#94a3b8', lineHeight: 1 }}>
            Property &amp; Vehicles
          </Typography>
        )}
      </Box>
  ...
  return (
    <Box sx={{
      ...
      <Box
        ...
        to="/subscription"
        sx={{
          ml: 1, fontSize: '0.75rem', fontWeight: 800, color: '#fff',
          display: 'inline-flex', alignItems: 'center', gap: 0.2,
          textDecoration: 'none',
          background: 'rgba(255,255,255,0.18)', backdropFilter: 'blur(8px)',
          px: 1.2, py: 0.35, borderRadius: '999px',
          border: '1px solid rgba(255,255,255,0.25)', transition: 'background .2s',
      ...
      <IconButton
        ...
        sx={{
          ...
          color: 'rgba(255,255,255,0.75)', p: 0.5,
...
export default function Navbar() {
  ...
  const navCfg        = NAV_CONFIG[roleKey]
  const dropdownItems = navCfg?.dropdown ?? []
  const showBar       = showAnnouncement && !user?.isPremium
  ...
  const handleLogout = () => { closeMenu(); closeMobile(); logout(); navigate('/') }

  return (
    <>
      {showBar && <AnnouncementBar onClose={() => setShowAnnouncement(false)} />}

      {/* ── Sticky wrapper — NO background so nothing leaks around the pill ── */}
      <Box
        sx={{
          position: 'sticky',
          top: 10,
          ...
          pt: 1.2,
          pb: 0.5,
          // ✅ FIX: explicitly transparent — the wrapper itself has no colour
      ...
      >
        <Container maxWidth="xl" disableGutters sx={{ pointerEvents: 'auto' }}>
          ...
          {/* ── Pill card ── ✅ FIX: opacity raised from 0.78→0.95 / 0.92→0.98
               so no body gradient bleeds through the semi-transparent surface   */}
          <Box
            sx={{
              borderRadius: '999px',
              background: scrolled
                ? 'rgba(255,255,255,0.98)'   // ✅ was 0.92
                : 'rgba(255,255,255,0.95)',   // ✅ was 0.78
            ...
            >
              ...
              {roleKey === 'guest' && (
                <>
                  <Button
                    ...
                    sx={{
                      fontWeight: 700, fontSize: '0.88rem', color: '#475569',
                      borderRadius: '999px', px: 2, py: 1,
                  ...
                  <Button
                    ...
                    variant="outlined"
                    sx={{
                      fontWeight: 700, fontSize: '0.88rem', borderRadius: '999px',
                      px: 2.2, py: 0.95,
                      borderColor: 'rgba(148,163,184,0.5)', color: '#475569',
                      '&:hover': { borderColor: '#7c6cff', color: '#5b4cf0', background: 'rgba(109,91,255,0.04)' },
                    }}
                  >
                    Login
                  </Button>
                  <Button
                    ...
                    variant="contained"
                    sx={{
                      fontWeight: 800, fontSize: '0.88rem', borderRadius: '999px',
                      px: 2.4, py: 1,
              ...
              {(roleKey === 'free' || roleKey === 'premium') && (
                <>
                  ...
                  {roleKey === 'premium' && (
                    <Button
                      component={RouterLink} to="/dashboard/add-property"
                      variant="outlined" startIcon={<SellRoundedIcon />}
                      sx={{
                        fontWeight: 700, fontSize: '0.88rem', borderRadius: '999px',
                        px: 2, py: 0.95,
                  ...
                  {roleKey === 'free' && (
                    <Button
                      component={RouterLink} to="/subscription"
                      variant="contained"
                      startIcon={<WorkspacePremiumRoundedIcon sx={{ fontSize: 16 }} />}
                      sx={{
                        fontWeight: 800, fontSize: '0.85rem', borderRadius: '999px',
                        px: 2.2, py: 0.95,
                  ...
                  <Tooltip title="Account menu" arrow>
                    <IconButton
                      ...
                      sx={{
                        ...
                        '&:hover': { borderColor: '#7c6cff', boxShadow: '0 0 0 3px rgba(109,91,255,0.12)' },
                      }}
                    >
                      <UserAvatar initials={initials} photo={user?.photo} roleKey={roleKey} size={36} />
                    </IconButton>
                  </Tooltip>
                </>
              )}
              ...
              {roleKey === 'admin' && (
                <>
                  ...
                  <Button
                    ...
                    variant="outlined" startIcon={<AdminPanelSettingsRoundedIcon />}
                    sx={{
                      fontWeight: 700, fontSize: '0.88rem', borderRadius: '999px',
                      px: 2, py: 0.95,
                      borderColor: 'rgba(239,68,68,0.35)', color: '#b91c1c',
                  ...
                  <Tooltip title="Admin account" arrow>
                    <IconButton
                      ...
                      sx={{
                        ...
                        background: 'rgba(255,255,255,0.9)',
                        '&:hover': { borderColor: '#ef4444', boxShadow: '0 0 0 3px rgba(239,68,68,0.10)' },
                      }}
                    >
      ...
      <Menu
        ...
        PaperProps={{
          elevation: 0,
          sx: {
            mt: 1.6, minWidth: 248, borderRadius: '22px',
            boxShadow: '0 24px 60px rgba(15,23,42,0.13), 0 4px 16px rgba(15,23,42,0.06)',
            ...
            '&::before': {
              ...
              borderBottom: 'none', borderRight: 'none', zIndex: 1,
            },
          },
        }}
      >
        ...
        >
          <Stack direction="row" spacing={1.5} alignItems="center">
            ...
            <Box>
              <Typography sx={{ fontWeight: 900, fontSize: '0.97rem', lineHeight: 1.2, color: '#0f172a' }}>
                {user?.name || 'User'}
              </Typography>
              <Typography sx={{ fontSize: '0.74rem', color: '#64748b', fontWeight: 500, mb: 0.6 }}>
                {user?.email || ''}
              </Typography>
              <RoleBadge roleKey={roleKey} />
            </Box>
        ...
        {dropdownItems.map((item) => (
          <MenuItem
            key={item.label}
            component={RouterLink} to={item.to} onClick={closeMenu}
            sx={{
              py: 1.4, px: 2.2, fontWeight: 600, fontSize: '0.88rem',
              color: '#334155', gap: 0.5,
              '&:hover': { background: 'rgba(109,91,255,0.05)', color: '#5b4cf0' },
            }}
          >
            <ListItemIcon sx={{ minWidth: 36, '& svg': { fontSize: 18, color: '#94a3b8' } }}>
              {item.icon}
            </ListItemIcon>
            {item.label}
        ...
        <MenuItem
          ...
          sx={{
            py: 1.4, px: 2.2, fontWeight: 700, fontSize: '0.88rem',
            color: '#ef4444', gap: 0.5, borderRadius: '0 0 22px 22px',
            '&:hover': { background: 'rgba(239,68,68,0.05)' },
          }}
        >
          <ListItemIcon sx={{ minWidth: 36, '& svg': { fontSize: 18, color: '#ef4444' } }}>
            <LogoutRoundedIcon />
          </ListItemIcon>
          Logout
        </MenuItem>
      ...
      <Drawer
        ...
        PaperProps={{
          sx: {
            width: 330, borderRadius: '28px 0 0 28px',
      ...
      >
        <Stack sx={{ height: '100%' }}>
          <Box sx={{ px: 2.5, pt: 2.5, pb: 2, borderBottom: '1px solid rgba(226,232,240,0.7)' }}>
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
              <Logo compact />
              <IconButton
                onClick={closeMobile}
                sx={{
                  border: '1.5px solid rgba(226,232,240,0.9)', borderRadius: '14px', p: 0.8,
            ...
            {loggedIn && (
              ...
              >
                <Stack direction="row" spacing={1.4} alignItems="center">
                  ...
                  <Box>
                    <Typography sx={{ fontWeight: 900, fontSize: '0.95rem', color: '#0f172a', lineHeight: 1.2 }}>
                      {user?.name || 'User'}
                    </Typography>
          ...
          <Box sx={{ flex: 1, overflowY: 'auto', px: 2, py: 1.5 }}>
            <Stack spacing={0.4}>
              {navCfg.mobile.map((item) => {
                ...
                return (
                  <Box
                    key={item.label}
                    component={RouterLink} to={item.to} onClick={closeMobile}
                    sx={{
                      display: 'flex', alignItems: 'center', gap: 1.3,
                      px: 1.8, py: 1.45, borderRadius: '18px',
                      ...
                      '&:hover': { background: 'rgba(109,91,255,0.06)', color: '#5b4cf0' },
                    }}
                  >
                    <Box
                      sx={{
                        ...
                        boxShadow: active ? '0 6px 16px rgba(109,91,255,0.25)' : 'none',
                        transition: 'all .2s',
                      }}
                    >
                      {item.icon}
                    ...
                    {item.label}
                    {active && (
                      <ChevronRightRoundedIcon sx={{ ml: 'auto', fontSize: 16, color: '#a5b4fc' }} />
                    )}
                  </Box>
                )
              })}
            ...
            {roleKey === 'free' && (
              <Box
                component={RouterLink} to="/subscription" onClick={closeMobile}
                sx={{
                  mt: 2, display: 'flex', alignItems: 'center', gap: 1.2,
                  p: 2, borderRadius: '18px', textDecoration: 'none',
                  background: 'linear-gradient(135deg, #f59e0b 0%, #f97316 100%)',
              ...
              >
                ...
                <Box>
                  <Typography sx={{ fontWeight: 900, fontSize: '0.88rem', color: '#fff', lineHeight: 1.2 }}>
                    Upgrade to Premium
                  </Typography>
                  <Typography sx={{ fontSize: '0.73rem', color: 'rgba(255,255,255,0.82)' }}>
                    Unlock contacts, prices &amp; post listings — ₹299
                  </Typography>
                </Box>
                <ChevronRightRoundedIcon sx={{ ml: 'auto', color: '#fff', fontSize: 18 }} />
              </Box>
            )}
          </Box>

          {loggedIn && (
            <Box sx={{ px: 2.5, py: 2, borderTop: '1px solid rgba(226,232,240,0.7)' }}>
              <Button
                fullWidth variant="outlined"
                startIcon={<LogoutRoundedIcon />}
                onClick={handleLogout}
                sx={{
                  py: 1.5, fontWeight: 700, borderRadius: '18px', fontSize: '0.88rem',
                  borderColor: 'rgba(239,68,68,0.30)', color: '#ef4444',
                  '&:hover': { borderColor: '#ef4444', background: 'rgba(239,68,68,0.05)' },
                }}
              >
                Logout
